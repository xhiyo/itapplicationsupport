...
export default function LaporkanKendala({ setActivePage, globalSearchTerm }) {
  const { t } = useLanguage();
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [localSearchTerm, globalSearchTerm, statusFilter, categoryFilter, picFilter]);
  ...
  const handleCreateOrUpdate = async (formData, id) => {
    try {
      ...
      axios.post("http://localhost:5000/api/tickets/log", {
  ...
  const handleInlineUpdate = async (ticket, field, value) => {
    try {
      ...
      await axios.put(`${API_URL}/${ticket.id}`, { [field]: value });
      invalidateCache('tickets');
      window.dispatchEvent(new Event('ticketsUpdated'));
    } catch (error) {
  ...
  // Derived metrics
  const openCount = tickets.filter(t => t.status === 'To Do').length;
  const progressCount = tickets.filter(t => t.status === 'On Progress').length;
  const completedCount = tickets.filter(t => t.status === 'Completed').length;
  const highPriorityCount = tickets.filter(t => t.priority === 'High' && t.status !== 'Completed').length;
  ...
  const filteredTickets = tickets.filter(ticket => {
    ...
    return matchesSearch && matchesStatus && matchesCategory && matchesPic;
  });
  ...
  return (
    <div className="flex h-full bg-slate-50 dark:bg-slate-900 relative overflow-hidden animate-in fade-in duration-500">
      ...
      >
        <div className="flex flex-col p-8 gap-6 min-h-full">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
            {/* Card 1 */}
            <div className="bg-white dark:bg-slate-800 rounded-[20px] p-5 border border-slate-100 dark:border-slate-700 shadow-[0_2px_10px_-3px_rgba(6,81,237,0.05)] hover:shadow-md transition-shadow">
              <div className="flex justify-between items-start mb-4">
                ...
                <h3 className="text-3xl font-black text-[#0b1b3d] dark:text-white">{openCount}</h3>
              </div>
              <div className="flex justify-between items-end">
                <div>
                  <p className="text-sm font-bold text-slate-800 dark:text-white">{t('Tiket To Do')}</p>
                  <p className="text-xs font-medium text-slate-400 dark:text-slate-500 mt-1">{t('Tiket belum/sedang penanganan')}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-slate-800 rounded-[20px] p-5 border border-slate-100 dark:border-slate-700 shadow-[0_2px_10px_-3px_rgba(6,81,237,0.05)] hover:shadow-md transition-shadow">
              <div className="flex justify-between items-start mb-4">
                <div className="w-12 h-12 rounded-xl bg-green-50 dark:bg-green-900/30 text-green-500 dark:text-green-400 flex items-center justify-center">
                  <CheckCircle2 size={24} strokeWidth={2.5} />
                </div>
                <h3 className="text-3xl font-black text-[#0b1b3d] dark:text-white">{completedCount}</h3>
              </div>
              <div className="flex justify-between items-end">
                <div>
                  <p className="text-sm font-bold text-slate-800 dark:text-white">{t('Tiket Selesai')}</p>
                  <p className="text-xs font-medium text-slate-400 dark:text-slate-500 mt-1">{t('Total tiket diselesaikan')}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-slate-800 rounded-[20px] p-5 border border-slate-100 dark:border-slate-700 shadow-[0_2px_10px_-3px_rgba(6,81,237,0.05)] hover:shadow-md transition-shadow">
              <div className="flex justify-between items-start mb-4">
                <div className="w-12 h-12 rounded-xl bg-red-50 dark:bg-red-900/30 text-red-500 dark:text-red-400 flex items-center justify-center">
                  <AlertCircle size={24} strokeWidth={2.5} />
                </div>
                <h3 className="text-3xl font-black text-[#0b1b3d] dark:text-white">{highPriorityCount}</h3>
              </div>
              <div className="flex justify-between items-end">
                <div>
                  <p className="text-sm font-bold text-slate-800 dark:text-white">{t('Prioritas Tinggi')}</p>
                  <p className="text-xs font-medium text-slate-400 dark:text-slate-500 mt-1">{t('Butuh tindakan segera')}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-slate-800 rounded-[20px] p-5 border border-slate-100 dark:border-slate-700 shadow-[0_2px_10px_-3px_rgba(6,81,237,0.05)] hover:shadow-md transition-shadow">
              <div className="flex justify-between items-start mb-4">
                ...
                <h3 className="text-3xl font-black text-[#0b1b3d] dark:text-white">{tickets.length}</h3>
              </div>
              <div className="flex justify-between items-end">
                <div>
                  <p className="text-sm font-bold text-slate-800 dark:text-white">{t('Total Laporan')}</p>
                  <p className="text-xs font-medium text-slate-400 dark:text-slate-500 mt-1">{t('Keseluruhan tiket')}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          {/* Table Container */}
          <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm rounded-2xl flex flex-col overflow-hidden shrink-0 min-h-0">
            ...
            <div className="p-4 border-b border-slate-100 dark:border-slate-700 flex flex-wrap gap-3 items-center bg-slate-50/50 dark:bg-slate-800/50">
              <div className="relative flex-1 min-w-[200px]">
                ...
                <input
                  type="text"
                  placeholder={t("Cari subject, deskripsi, atau ID...")}
                  value={localSearchTerm}
                  onChange={(e) => setLocalSearchTerm(e.target.value)}
                  className="w-full pl-10 pr-4 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 outline-none transition-all dark:text-white"
                />
              </div>
              ...
              <div className="relative min-w-[150px]">
                <select
                  ...
                  className="appearance-none w-full pl-9 pr-10 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-semibold text-slate-700 dark:text-slate-300 focus:border-blue-500 focus:bg-white outline-none cursor-pointer"
                >
                  <option value="Semua PIC">{t('Semua PIC')}</option>
                  {[...new Set(pics.map(p => p.pic_name))].filter(Boolean).map((picName, idx) => (
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-slate-200 dark:border-slate-700 bg-slate-50/80 dark:bg-slate-800/80">
                    <th className="py-3.5 px-4 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider w-[8%]">ID</th>
                    <th className="py-3.5 px-4 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider w-[24%]">{t('Subject')}</th>
                    <th className="py-3.5 px-4 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider w-[12%]">{t('Layanan')}</th>
                    <th className="py-3.5 px-4 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider w-[15%]">{t('PIC')}</th>
                    <th className="py-3.5 px-4 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider w-[12%]">{t('Status')}</th>
                    <th className="py-3.5 px-4 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider w-[10%]">{t('Prioritas')}</th>
                    <th className="py-3.5 px-4 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider w-[12%]">{t('Tanggal')}</th>
                    <th className="py-3.5 px-4 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider text-right w-[7%]">{t('Aksi')}</th>
                  </tr>
                </thead>
                <tbody key={`page-${currentPage}`} className="divide-y divide-slate-100 dark:divide-slate-700 animate-in fade-in slide-in-from-bottom-2 duration-300">
                  {loading ? (
                    <tr><td colSpan="8" className="py-12 text-center text-slate-500 dark:text-slate-400 font-medium">Loading tickets...</td></tr>
                    ...
                    <>
                      {currentTickets.map((ticket) => {
                        ...
                        return (
                          <tr key={ticket.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-700/50 transition-colors group bg-white dark:bg-slate-800">
                            <td
                              className="px-6 py-4 align-middle cursor-pointer"
                              onClick={() => setActivePage(`TicketDetail_${ticket.id}`)}
                            >
                              <div className="text-sm font-semibold text-slate-800 dark:text-slate-200 line-clamp-1 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
                                {ticket.title}
                              </div>
                            ...
                            <td className="px-6 py-4 align-middle">
                              <div className="flex items-center gap-3">
                                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white font-bold text-[11px] shadow-sm ${ticket.pic_name ? getAvatarColor(ticket.pic_name) : 'bg-slate-200 text-slate-500'}`}>
                                  {getInitials(ticket.pic_name || 'NA')}
                                </div>
                                <span className="font-bold text-sm text-slate-800 dark:text-white">{ticket.pic_name || t('Belum di-assign')}</span>
                              </div>
                            </td>
                            ...
                            <td className="px-6 py-4 align-middle">
                              {ticket.priority === 'High' ? (
                                <span className="inline-flex items-center text-xs font-bold text-red-500 bg-red-50 px-2 py-1 rounded">
                                  {t('High')}
                                </span>
                                ...
                                <span className="inline-flex items-center text-xs font-bold text-emerald-500 bg-emerald-50 px-2 py-1 rounded">
                                  {t('Low')}
                                </span>
                              )}
                            </td>
                            ...
                            <td className="px-6 py-4 align-middle text-center">
                              <div className="flex items-center justify-center gap-2">
                                <button
                                  onClick={() => ticket.status === 'Completed' ? handleViewResolution(ticket) : (() => { setClosingTicket(ticket); setResolutionText(""); })()}
                                  className={`p-1.5 rounded-md transition-all ${ticket.status === 'Completed'
                                  ...
                                  title={ticket.status === 'Completed' ? t("Lihat Solusi") : t("Selesaikan Tiket")}
                                >
                                  <CheckCircle size={16} />
                                </button>
                                <button
                                  onClick={(e) => ticket.status !== 'Completed' && openEditModal(e, ticket)}
                                  disabled={ticket.status === 'Completed'}
                                  className={`p-1.5 rounded-md transition-all ${ticket.status === 'Completed' ? 'text-slate-300 dark:text-slate-700 cursor-not-allowed opacity-50' : 'text-slate-400 dark:text-slate-500 hover:text-blue-500 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/30'}`}
                                  title={ticket.status === 'Completed' ? t("Tiket yang sudah selesai tidak dapat diedit") : t("Edit Ticket")}
                                >
                                  <Pencil size={16} />
                                </button>
                                <button
            ...
            {/* Pagination Controls */}
            {!loading && (
              <div className="p-4 border-t border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 flex items-center justify-between rounded-b-2xl">
                <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-700 px-3 py-1.5 rounded-lg">
                ...
                {totalPages > 1 && (
                  <div className="flex items-center gap-1">
                    <button
                      ...
                      className="w-8 h-8 flex items-center justify-center rounded-md border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors"
      ...
      {isModalOpen && createPortal(
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-900/40 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-2xl h-fit max-h-[90vh] overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
            <TicketModal
              isOpen={isModalOpen}
      ...
      {closingTicket && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-slate-900/60 p-4">
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700 max-w-lg w-full overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center bg-slate-50 dark:bg-slate-900">
              <h3 className="font-bold text-slate-800 dark:text-white text-base">Selesaikan Tiket #{closingTicket.id}</h3>
            </div>
            <div className="p-6">
              ...
              <textarea
                ...
                className="w-full border border-slate-200 dark:border-slate-700 rounded-lg p-3 text-sm focus:ring-2 focus:ring-green-500 outline-none resize-none bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-500"
                rows={4}
              ...
              <div className="flex gap-3 mt-6">
                <button
                  ...
                  className="flex-1 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 px-4 py-2.5 rounded-lg font-semibold hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors"
                >
                  Batal
                </button>
                <button
                  onClick={submitCloseTicket}
                  className="flex-1 bg-green-600 text-white px-4 py-2.5 rounded-lg font-semibold hover:bg-green-700 transition-colors shadow-sm flex items-center justify-center gap-2"
                >
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700 max-w-lg w-full overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center bg-slate-50 dark:bg-slate-900">
              <h3 className="font-bold text-slate-800 dark:text-white text-base flex items-center gap-2">
                ...
                Solusi Tiket #{viewingResolutionTicket.id}
              </h3>
            </div>
            <div className="p-6">
              {resolutionData.loading ? (
                ...
                <div className="bg-green-50 dark:bg-green-900/20 text-green-900 dark:text-green-300 p-4 rounded-xl border border-green-100 dark:border-green-800/30 whitespace-pre-wrap text-sm leading-relaxed">
                  {resolutionData.text}
                </div>
              )}
              <div className="flex mt-6">
                <button
                  ...
                  className="w-full bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 px-4 py-2.5 rounded-lg font-semibold hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors"
                >
                  Tutup
                </button>
              </div>
      ...
      {deletingTicket && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-slate-900/60 p-4">
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700 max-w-md w-full overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center bg-slate-50 dark:bg-slate-900">
              <h3 className="font-bold text-slate-800 dark:text-white text-base flex items-center gap-2">
            ...
            <div className="p-6">
              <p className="text-slate-600 dark:text-slate-300 text-sm mb-6">
                {t('Apakah Anda yakin ingin menghapus tiket')} <strong className="text-slate-800 dark:text-white">#{deletingTicket.id} - {deletingTicket.title}</strong> {t('secara permanen? Tindakan ini tidak dapat dibatalkan.')}
              </p>
              <div className="flex gap-3">
                ...
                <button
                  ...
                  className="flex-1 bg-rose-600 text-white px-4 py-2.5 rounded-lg font-semibold hover:bg-rose-700 transition-colors shadow-sm flex items-center justify-center gap-2"
                >
